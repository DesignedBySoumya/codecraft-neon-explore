...
const Leaderboard = () => {
  ...
  return (
    <div className="glass-card p-6 rounded-xl">
      ...
      <div className="flex space-x-1 mb-6 bg-secondary/50 p-1 rounded-lg">
        {tabs.map((tab) => (
          <Button
            ...
            className={`flex-1 ${activeTab === tab.key ? 'bg-neon-green text-black font-semibold' : 'text-muted-foreground'}`}
          >
            {tab.label}
          </Button>
        ))}
      ...
      <div className="space-y-3">
        {leaderboardData[activeTab].map((user, index) => (
          <motion.div
            ...
            className={`flex items-center gap-4 p-3 rounded-lg transition-all duration-300 hover:bg-secondary/50 ${
              user.rank <= 3 ? 'bg-gradient-to-r from-neon-green/5 to-transparent border border-neon-green/20' : ''
            }`}
          >
            {/* Rank */}
      ...
      >
        <Button variant="outline" className="w-full border-neon-green/30 hover:border-neon-green hover:shadow-neon">
          View Full Rankings
        </Button>
      </motion.div>
    </div>
